...
struct FMinimapTarget
{
	...
	UPROPERTY()
	AActor* TargetActor; // 추적할 액터 (다른 플레이어, 깃발, 버튼 등)

	UPROPERTY()
	UUserWidget* TargetIconWidget; // 화면에 띄워진 아이콘 위젯
};
...
public:
	// 미니맵에 새로운 타겟을 등록하는 함수
	UFUNCTION(BlueprintCallable, Category = "Minimap")
	void AddMinimapTarget(AActor* InActor, TSubclassOf<UUserWidget> IconClass);
